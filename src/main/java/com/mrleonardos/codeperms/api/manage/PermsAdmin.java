 ...
 *
 * <p>
 * Правка строится на снимке, который метод взял в начале работы. Если параллельная правка успела
 * уйти в модель раньше, вторая не применяется поверх первой: она отклоняется кодом
 * {@code STALE_SNAPSHOT}, хранилище остаётся нетронутым, и повтор на свежем снимке безопасен. Правки
 * из одного потока, включая команды, друг друга не опережают.
 *
 * <p>
...
public interface PermsAdmin {
     ...
     * прежнюю. Если целевая группа уже была выдана и у обеих выдач есть срок, остаётся более поздний:
     * так меньше риск случайно отобрать право. Бессрочная выдача позднее любой срочной.
     *
     * <p>
