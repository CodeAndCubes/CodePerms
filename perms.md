...
сообщения под `codeperms.message.*`, описания нод для каталога под `codeperms.node.*`, всё остальное
под `codeperms.` это ноды из таблиц ниже. Собственные ноды мод регистрирует в каталоге
`PermsApi.catalog()` при сборке, поэтому автодополнение нод работает сразу после установки.
